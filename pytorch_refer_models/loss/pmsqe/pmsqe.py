...
import pathlib
import os
import utils.conv_stft as conv_stft
...
class PMSQELoss(nn.Module):
    def __init__(self, hop=128, log_mse=False):
        super(PMSQELoss, self).__init__()
        self.pmsqe = SingleSrcPMSQE()
        self.stft = conv_stft.ConvSTFT(512, hop, 512, "hanning", 'complex')
        self.log_mse = log_mse
        self.mse = nn.MSELoss()

    def forward(self, est_sig, clean_sig, training=False):
        est_power = power(self.stft(est_sig))
        clean_power = power(self.stft(clean_sig))
        mse = 0
        if self.log_mse:
            mse = self.mse(torch.log(est_power + 1e-8), torch.log(clean_power + 1e-8))
        return self.pmsqe(est_power, clean_power).mean() + mse


def power(x):
    real, imag = torch.chunk(x, 2, dim=1)
    return (real ** 2 + imag ** 2).permute(0, 2, 1)
